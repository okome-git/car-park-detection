...
def run(
    ...
    with open("./position_label.txt","r") as file:
        position_label = file.readlines()
    position_data = {}
    for position in position_label:
        cls_label, x_label, y_label, w_label, h_label = position.split(' ')
        position_x1 = float(x_label) - (float(w_label)/2)
        position_x2 = float(x_label) + (float(w_label)/2)
        position_y1 = float(y_label) - (float(h_label)/2)
        position_y2 = float(y_label) + (float(h_label)/2)
        position_cls = position_classes[int(cls_label)]
        position_data[position_cls] = [position_x1,position_x2,position_y1,position_y2]
    #print(position_data)
    ################################################################
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    position_y1 = xywh[1] - (xywh[3]/2)
                    position_y2 = xywh[1] + (xywh[3]/2)
